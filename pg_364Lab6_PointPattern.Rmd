...
```{=html}
<p class="comment"><strong>THIS WILL ONLY WORK IF YOU HAVE RUN THE LOAD LIBRARY CODE CHUNK TO LOAD THE SPATSTAT PACKAGE. If you see a list of stuff and it doesn't make a map, install and load library(spatstat) </strong></p>
```
<br><br>
...
### B1. Understanding schools across NY State
...
### B4. Converting data into ppp format

Spatstat does require us to convert all our data into specific 'custom' formats.

1.  First we need to turn our NY state border into a special spatial "owin" frame, This will allow us to make our spatial domain NY state rather than a rectangle.Get the code working below in your own lab.
...
2.  We are going to work with *unmarked* data (for now). So for each of Schools and McDonalds, we need to first only use the geometry (X/Y locations) and then convert it to a ppp format, then assign our window as New York. Get the code working below in your own lab. You should see a version of the map where it's easier to see the point patterns.
...
3.  Repeat for the McDonalds data to make a ppp version of that. You should see something like this.
...
To be updated by 10:15. Refresh your page!

If you are really up to here (already?), read this while you're waiting as we're working through something similar
