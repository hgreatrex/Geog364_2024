...
A basic measure of a pattern’s density is its overall, or **global** **density**. E.g. how many points are there per unit area, ( e.g. number of penguins per square mile, or number of flowers per square-foot)

Global_density = number_points / study_area
...
2.  See if you can adjust the bandwidth for Schools and McDonald's to say something interesting about each pattern, linking it to the underlying geography/environment (e.g. these are real school/Mcdonalds locations, what's going on) Explain your process/ reasoning below your maps. <br><br>
...
Hello from Dr G! First, I have finally found a way for you to better see your plots below a code chunk - I know the ppp ones have been difficult to make out as they appear tiny. What we do is change the margin of the plot using the "plotting parameters" `par()` command BEFORE making the plot. There are MANY things you can change using this command\
...
As I play with my quadrat size, I seem to get different answers about where schools are. For example:

```{r, include=FALSE}

# Plot the intensity
par(oma=c(0,0,0,0),mar=c(.5,.5,1,4),cex.main=.9,mfrow=c(1,2))
...
Quad.School <- quadratcount(SchoolWest.ppp, nx = 12, ny = 3)
plot(intensity(Quad.School, image=TRUE),
     col=my_colours, 
     main="Point intensity of schools", las=1)  
plot(west.owin,add=TRUE)
...
-   **[TASK] - According to my first plot plot above, I think that there should be most points in the West. In the second it looks like I should go north.. I'm guessing yours says something different again.... \
    \
    What effect are we describing here and why is it a problem for quadrat analysis? (hint, in the lecture notes and linked to gerrymandering)**
...
-   **[Advanced\*\* \*\*TASK]** Go back to section A1 and B4 and take a look at your schools points. What nuance is your test result missing e.g. do you agree with its results at every scale and location? By this, I mean - quadrat test is a global statistic. What are the pros and cons of this vs local statistics?
...
Here is my example for schools, but you do not need to analyse this.
